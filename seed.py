...
def inference(prompt: str) -> str:
    ...
    artifact = {"type": "cognitive_artifact", "prompt": prompt, "response": res, "latency": end_t - start_t, "timestamp": time.time()}
    h = hashlib.sha256(json.dumps(artifact).encode()).hexdigest()
    path = VAULT_DIR / h
    with open(path, 'w') as f: json.dump(artifact, f)
    ref_path = VAULT_DIR / "cognitive" / h
    ref_path.parent.mkdir(parents=True, exist_ok=True)
    with open(ref_path, 'w') as f: f.write(h)
    return res
...
def _raw_get(h: str, is_bios: bool = False) -> str:
    if is_bios or not os.path.exists("manifest.hash"):
        path = VAULT_DIR / h
        if not path.exists() or not path.is_file(): raise FileNotFoundError(f"Blob {h} not found in BIOS.")
    ...
    if not l2_h or h == l2_h: return _raw_get(h, is_bios=True)
    try:
        l2_payload = _raw_get(l2_h, is_bios=True)
        l2_scope = {"context": {"target": h, "vault_tiers": ["blob_vault", "remote_vault", "collective_vault"]}, "log": lambda m: None, "result": None, "embed": embed, "rerank": rerank, "inference": inference, "json": json, "os": os, "glob": glob}
        exec(l2_payload, l2_scope, l2_scope)
        resolved_path = l2_scope.get("result", {}).get("path")
        if resolved_path and os.path.exists(resolved_path):
            with open(resolved_path, "r") as f: return f.read()
...
def _invoke_jury(proposal_id: str) -> dict:
    ...
    try:
        ...
        jury_scope = {"context": {"proposal_id": proposal_id}, "log": lambda m: None, "result": None, "inference": inference, "sign": sign, "tally": tally, "_raw_get": _raw_get, "json": json, "os": os, "__builtins__": __builtins__}
...
def invoke(h: str, context: dict = None) -> dict:
    ...
    if proxy_h and h != proxy_h:
        try:
            ...
            l1_scope = {"context": {"request": context, "timestamp": time.time(), "trace_id": "syn-" + hashlib.sha256(str(time.time()).encode()).hexdigest()[:8]}, "log": log, "result": None, "__builtins__": __builtins__, "inference": inference, "embed": embed}
    ...
    if broker_h and h != broker_h:
        try:
            ...
            l3_scope = {
                ...
                "log": log, "result": None, 
                "os": os, "glob": glob, "json": json, "hashlib": hashlib, "__builtins__": __builtins__,
                "inference": inference, "embed": embed, "rerank": rerank, "put": put, "propose": propose}
    ...
    try:
        ...
        if engine_h and h != engine_h:
            ...
            l4_scope = {"context": {"target_payload": payload, "target_context": request_envelope, "execution_plan": execution_plan, "state": state}, "log": log, "result": None, "subprocess": subprocess, "json": json, "__builtins__": __builtins__, "inference": inference, "embed": embed}
...
def main():
    ...
    if cmd == "put":
        ...
        print(f"Signed {sys.argv[2]} as {node}: {sign(sys.argv[2], node)}")
    elif cmd == "governance":
        ...
        if not proposal_path.exists():
            ...
            if tally(proposal_id) < min_sigs:
                print(f"FAILED: Consensus not reached for {proposal_id}"); sys.exit(1)
