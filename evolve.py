...
f_9: adversarial test/case authorship — two-model inference pipeline.

Cycle per core blob:
  1. Evaluate:  read current hash from manifest, get fitness signals from telemetry
  2. Implement: call `ai gemini-flash` to generate candidate (implementor pass)
                Falls back to deterministic v8 mutation if ai is unavailable.
  3. Review:    Triple-Pass (Pass 1–5 + Pass 6 ContractCompliance)
  4. Test:      call `ai devstral` to generate adversarial test/case blobs (tester pass)
                Promote test cases; run test suite against candidate.
                Reject candidate if any test fails.
  5. Benchmark: invoke old vs new N times, compare measured fitness
  6. Promote:   if new wins, Council Approval → manifest update (v1.9.0)
...
# Mutation goals passed to the inference prompt when ai binary is available.
_MUTATION_GOALS: dict[str, str] = {
    "discovery": (
        "Improve resilience: handle missing vault directory gracefully, "
        "validate hash format before attempting file read, add structured "
        "error context to exceptions."
    ),
    "planning": (
        "Improve selection quality: account for invocation_count confidence "
        "weighting so blobs with few samples are treated conservatively; "
        "ensure feedback_score influences selection more directly."
    ),
    "telemetry-reader": (
        "Add p99 latency estimation alongside p95 using Welford-variant "
        "online algorithm; expose invocation_count trend (recent vs historical) "
        "to help planning distinguish improving from declining blobs."
    ),
}
...
def evolve_one(label: str, reviewer: str = "evolve") -> dict:
    ...
    # Generate candidate — inference first, deterministic mutation fallback
    generated_via_inference = False
    ...
    goal = _MUTATION_GOALS.get(label, f"Improve {label} blob performance and correctness.")
    ...
    # Tester pass — adversarial test/case generation (different model from implementor)
    tester_model = "devstral"
    test_case_hashes: list[str] = []
    test_suite_results: list[dict] = []
    try:
        ...
        for tc_payload in tc_payloads:
            ...
            tc_hash = seed.put("test/case", _json.dumps(tc_payload, sort_keys=True))
            test_case_hashes.append(tc_hash)

        if test_case_hashes:
            tc_approval = promote.issue_council_approval(
                test_case_hashes, reviewer_hash=evolve_reviewer_hash
            )
            promote.promote_test_cases(label, test_case_hashes, tc_approval)
            print(f"  tester  {len(test_case_hashes)} test cases promoted ({tester_model})")

            # Run test suite against candidate
            test_suite_results = promote.run_test_suite(label, candidate_hash)
            failed = [r for r in test_suite_results if not r["passed"]]
            if failed:
                print(f"  FAIL  test suite: {len(failed)}/{len(test_suite_results)} failed")
                return {
                    "label":              label,
                    "outcome":            "test_suite_failed",
                    "failed_tests":       len(failed),
                    "total_tests":        len(test_suite_results),
                    "generated_via_inference": generated_via_inference,
                }
            print(f"  PASS  test suite: {len(test_suite_results)} cases passed")

    except infer.InferenceUnavailable as e:
        print(f"  tester unavailable ({e!s:.60}) — skipping test suite")
    except Exception as e:
        print(f"  tester error ({e!s:.80}) — skipping test suite")
...
def run_all(reviewer: str = "evolve") -> list[dict]:
    ...
    tolerance = _derive_tolerance()
    print(f"── f_9: Self-Modification Cycle  (derived tolerance={tolerance:.4f})")
    results = []
    for label in ["telemetry-reader", "planning", "discovery"]:
        # telemetry-reader first so subsequent benchmarks use v8 signals
        r = evolve_one(label, reviewer=reviewer)
        results.append(r)
    promoted = [r for r in results if r["outcome"] == "promoted"]
    inferred = [r for r in promoted if r.get("generated_via_inference")]
    print(f"\n  f_9 complete. {len(promoted)}/3 blobs promoted → manifest v1.9.0  "
          f"({len(inferred)} via inference)")
    return results
