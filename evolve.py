...
f_10: guided mutation goals + multi-candidate generation (N=3).

Cycle per core blob:
  1. Evaluate:  read current hash from manifest, get fitness signals from telemetry
  2. Goal:      derive mutation goal from fitness signal trends (audit log)
  ...
  7. Promote:   best winner → Council Approval → manifest update (v1.10.0)
...
def _derive_mutation_goal(label: str, signals: dict) -> str:
    """
    Derive a mutation goal from current fitness signal trends.

    Reads recent benchmark events for the label from the audit log to understand
    trend direction, then combines with current signal values to produce a
    targeted goal string for the implementor LLM.
    """
    sr  = signals.get("success_rate",    1.0)
    lat = signals.get("avg_latency_ms",  0.0)
    vol = signals.get("volatility",      0.0)

    # Read recent benchmark events to detect trend direction
    trend_note = ""
    try:
        audit_path = Path("./audit.log")
...
def run_all(reviewer: str = "evolve") -> list[dict]:
    ...
    tolerance = _derive_tolerance()
    print(f"── f_10: Self-Modification Cycle  (derived tolerance={tolerance:.4f})")
    results = []
    ...
    print(f"\n  f_10 complete. {len(promoted)}/3 blobs promoted → manifest v1.10.0")
    return results
