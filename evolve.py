...
f_8: inference-generated candidates via `ai` CLI binary (Cloudflare AI Gateway).

Cycle per core blob:
  1. Evaluate: read current hash from manifest, get fitness signals from telemetry
  2. Generate: call `ai` binary with current payload + contract + fitness signals
               Falls back to deterministic v8 mutation string if ai is unavailable.
  3. Review:   Triple-Pass (Pass 1–5 existing + Pass 6 ContractCompliance)
  4. Benchmark: invoke old vs new N times, compare measured fitness
  5. Promote:  if new wins, Council Approval → manifest update (v1.8.0)

Credential safety: CF_ACCOUNT_ID / CF_GATEWAY_NAME / CF_AIG_TOKEN live in
os.environ and are consumed by the `ai` subprocess.  They never appear in
subprocess arguments, vault blobs, or audit log entries.
"""
...
def run_all(reviewer: str = "evolve") -> list[dict]:
    ...
    tolerance = _derive_tolerance()
    print(f"── f_8: Self-Modification Cycle  (derived tolerance={tolerance:.4f})")
    results = []
    ...
    print(f"\n  f_8 complete. {len(promoted)}/3 blobs promoted → manifest v1.8.0  "
