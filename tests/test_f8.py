...
def test_full_f8_evolution_cycle() -> None:
    ...
    check("manifest version 1.10.0", m["version"] == "1.10.0")
    ...
    # Audit log must have v1.10.0 promote events (f_9 evolve.py promotes to v1.10.0)
    entries = [json.loads(l) for l in Path("./audit.log").read_text().splitlines() if l.strip()]
    promote_v9 = [e for e in entries
                  if e.get("event") == "promote" and e.get("version") == "1.10.0"]
    check("at least one v1.10.0 promote event in audit log", len(promote_v9) >= 1)
