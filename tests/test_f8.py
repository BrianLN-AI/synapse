...
def test_evolve_one_uses_fallback_when_inference_unavailable() -> None:
    section("evolve_one — falls back to deterministic mutation when inference unavailable")
    with mock.patch("infer.generate_candidate",
                    side_effect=infer.InferenceUnavailable("test: ai not available")):
        result = evolve.evolve_one("planning")
    # Should complete without error (using v8 mutation as fallback)
    check("outcome is not error", result["outcome"] in {"promoted", "no-improvement"})
    check("generated_via_inference is False",
          result.get("generated_via_inference") is False or
...
def test_full_f8_evolution_cycle() -> None:
    ...
    def mock_generate_ordered(current_payload, contract, fitness, mutation_goal, model="haiku"):
        ...
        for label in ["telemetry-reader", "planning", "discovery"]:
            if label in mutation_goal:
                return _payloads[label]
    ...
    check("manifest version 1.8.0", m["version"] == "1.8.0")
    ...
    # Audit log must have v1.8.0 promote events
    entries = [json.loads(l) for l in Path("./audit.log").read_text().splitlines() if l.strip()]
    promote_v8 = [e for e in entries
                  if e.get("event") == "promote" and e.get("version") == "1.8.0"]
    check("at least one v1.8.0 promote event in audit log", len(promote_v8) >= 1)
