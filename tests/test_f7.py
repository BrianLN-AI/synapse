...
Covers: contract/definition blob type, Pass 6 ContractCompliance, promote_contract(),
        ...
        detection, test/case blob type, full f_7 evolution cycle, manifest v1.9.0.
"""
...
def test_full_f7_evolution_cycle() -> None:
    section("Full f_7 evolution cycle — ContractCompliance active, manifest v1.9.0")
    # Force inference unavailable so evolve_one falls back to deterministic _MUTATIONS
    # and the test suite is skipped. Keeps backward-compat test hermetic (no live LLM).
    _iu = infer.InferenceUnavailable("ai binary not found (mocked)")
    ...
    m = promote.load_manifest()
    check("manifest version 1.9.0", m["version"] == "1.9.0")
    for label in ["discovery", "planning", "telemetry-reader"]:
    ...
    promote_v7 = [e for e in entries
                  if e.get("event") == "promote" and e.get("version") == "1.9.0"]
    check("at least one v1.9.0 promote event in audit log", len(promote_v7) >= 1)
