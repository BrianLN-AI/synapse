...
Covers: contract/definition blob type, Pass 6 ContractCompliance, promote_contract(),
        ...
        detection, test/case blob type, full f_7 evolution cycle, manifest v1.7.0.
"""
...
def test_full_f7_evolution_cycle() -> None:
    section("Full f_7 evolution cycle — ContractCompliance active, manifest v1.7.0")
    ...
    promote_v7 = [e for e in entries
                  if e.get("event") == "promote" and e.get("version") == "1.7.0"]
    check("at least one v1.7.0 promote event in audit log", len(promote_v7) >= 1)
